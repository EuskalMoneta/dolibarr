...
class Categories extends DolibarrApi
{
	 ...
	 */
	function get($id, $include_childs = false)
	{
	 ...
	 */
	function getListForObject($id, $type, $sortfield = "s.rowid", $sortorder = 'ASC', $limit = 0, $page = 0)
	{
		if (!in_array($type, [
		...
		if ($type == Categorie::TYPE_PRODUCT && !(DolibarrApiAccess::$user->rights->produit->lire || DolibarrApiAccess::$user->rights->service->lire)) {
			throw new RestException(401);
		} elseif ($type == Categorie::TYPE_CONTACT && !DolibarrApiAccess::$user->rights->societe->contact->lire) {
			throw new RestException(401);
	 ...
	 */
	function linkObjectById($id, $type, $object_id)
	{
	 ...
	 */
	function linkObjectByRef($id, $type, $object_ref)
	{
	 ...
	 */
	function unlinkObjectById($id, $type, $object_id)
	{
		if (empty($type) || empty($object_id)) {
			throw new RestException(401);
		}
	 ...
	 */
	function unlinkObjectByRef($id, $type, $object_ref)
	{
		if (empty($type) || empty($object_ref)) {
			throw new RestException(401);
		}
	 ...
	 * @url GET {id}/objects
	 */
	function getObjects($id, $type, $onlyids = 0)
	{
