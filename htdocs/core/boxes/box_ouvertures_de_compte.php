...
class box_ouvertures_de_compte extends ModeleBoxes
{
	...
	function loadBox()
	{
		...
		if ($user->rights->adherent->lire)
		{
			...
			$sql_comptes.= " FROM ".MAIN_DB_PREFIX."adherent_extrafields";
			$sql_comptes.= " WHERE documents_pour_ouverture_du_compte_valides = 1";

			$result_comptes = $db->query($sql_comptes);
			...
			if ($result_comptes && $result_prelevements)
			{
				$obj_comptes = $db->fetch_object($result_comptes);
				$obj_prelevements = $db->fetch_object($result_prelevements);

				$this->info_box_contents[0][] = array(
					'td' => 'align="left"',
					'text' => $langs->trans("NombreComptesOuverts"),
				);
				...
				$this->info_box_contents[3][] = array(
					'td' => 'align="left"',
